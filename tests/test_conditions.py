...
def test_parse_condition():
    ...
    print(visit_condition(result, {}).define())
    assert (
        visit_condition(result, {}).define() == (m.payload.text != "").define()
    )
    ...
    assert (
        ...
        == ((m.x == "foo") & (m.y == "bar")).define()
    )

    result = parse_condition(
        'events.first << event.x == "foo" and event.y == "bar"'
    )[0]
    print(result)
    print(visit_condition(result, {}).define())
    assert (
    ...
    result = parse_condition(
        'events.first << event.payload.src_path == "{{src_path}}"'
    )[0]
    print(result)
    print(visit_condition(result, {"src_path": "x"}).define())
    assert (
        visit_condition(result, {"src_path": "x"}).define()
        == (c.first << (m.payload.src_path == "x")).define()
    )
    ...
    assert (
        visit_condition(result, {"repo_name": "x"}).define()
        == (
            (m.payload.repository.full_name == "x") & (+m.payload.after)
        ).define()
    )

    result = parse_condition(
        "((event.x == 5) and (event.y == 6)) and (event.z == 7)"
    )
    print(result)
    print(visit_condition(result, {}).define())
    assert (
    ...
    result = parse_condition(
        "event.process_check.pid == facts.process_pid.pid"
    )
    print(result)
    print(visit_condition(result, {}))
    print(
        (
            m.process_check.pid
            == c.__getattr__("process_pid").__getattr__("pid")
        ).define()
    )
    print(visit_condition(result, {}).define())
    assert (
